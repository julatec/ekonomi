...
@Configuration(value = StorageConfig.CONFIGURATION)
@EnableTransactionManagement
@EnableJpaRepositories(
        entityManagerFactoryRef = StorageConfig.ENTITY_MANAGER_FACTORY,
        transactionManagerRef = StorageConfig.TRANSACTION_MANAGER,
        basePackageClasses = {
                name.julatec.ekonomi.tribunet.storage.Factura.class,
                name.julatec.ekonomi.accounting.AccountingPeriod.class
        }
)
public class StorageConfig {

    public static final String PERSISTENCE_UNIT = "ekonomiStorage";
    ...
    public DataSource multiRoutingDataSource() throws NamingException {
        ...
        for (Map.Entry<String, String> datasourceEntry : dataSources.entrySet()) {
            DataSource dataSource = (DataSource) new JndiTemplate().lookup(datasourceEntry.getValue());
            targetDataSources.put(datasourceEntry.getKey(), dataSource);
        }
        final MultiRoutingDataSource multiRoutingDataSource = new MultiRoutingDataSource();
        multiRoutingDataSource.setDefaultTargetDataSource(targetDataSources.get(defaultTenant));
