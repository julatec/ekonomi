...
public class MensajeReceptorCommand extends DocumentCommand<MensajeReceptorCommand> {

    private final MensajeReceptor mensaje;

    @Autowired
    MensajeReceptorMapper mapper;

    @Autowired
    MensajeRepository repository;

    public MensajeReceptorCommand(Context<?> context, Document document, MensajeReceptor mensaje) {
    ...
    public void run() {
        repository.forAll(getTenants(), repository -> {
            ...
            mensaje.setMensajeReceptor(super.getStringFromDocument());
